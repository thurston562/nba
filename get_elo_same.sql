WITH RECURSIVE p(current_game_number) AS (
  ...
  (
    ...
    SELECT
      game_sum_view_elo_same.game_number,
      home_team_name,
    ...
    FROM game_sum_view_elo_same
      JOIN previous_elos
        ON current_game_number = game_sum_view_elo_same.game_number - 1
      JOIN LATERAL (
           SELECT
             pow(10.0, (SELECT new_elo
                        FROM previous_elos
                        WHERE current_game_number = game_sum_view_elo_same.game_number - 1 AND home_team_name = home_team) / 400.0) AS r1,
             pow(10.0, (SELECT new_elo
                        FROM previous_elos
                        WHERE current_game_number = game_sum_view_elo_same.game_number - 1 AND home_team_name = visitor_team) / 400.0) AS r2
           ) r
        ON TRUE 
  )
)
